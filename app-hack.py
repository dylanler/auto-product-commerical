...
def generate_video_script(video_outputs, b_roll_metadata, audio_output, product_description, lora_outputs):
    ...
    # Describe LoRA output images
    lora_descriptions = describer.describe_lora_outputs(lora_outputs)
    ...
    content += "Product shoot videos and LoRA generated images descriptions:\n"
    for i, (video, description) in enumerate(zip(video_outputs, lora_descriptions)):
        if video:
...
def stitch_new_video(video_outputs, audio_output, product_description, lora_outputs):
    # Filter out None values from video_outputs
    valid_video_outputs = [v for v in video_outputs if v is not None]
    
    b_roll_metadata = 'b_roll_metadata'

    print("Valid video outputs: ", valid_video_outputs)
    
    if not valid_video_outputs:
        print("No valid video outputs found.")
        return None

    video_file_json = generate_video_script(valid_video_outputs, b_roll_metadata, audio_output, product_description, lora_outputs)
    video_files = video_file_json["video_sequence"]
    video_files = video_files[:random.randint(5, 8)]
    ...
    # Add the ending scene
    ending_scene_path = "cerebral-beach-ending.mp4"
    if os.path.exists(ending_scene_path):
        video_files.append(ending_scene_path)
    else:
        print(f"Warning: Ending scene '{ending_scene_path}' not found.")
...
with gr.Blocks() as demo:
    ...
    stitch_new_btn.click(
        lambda *args: stitch_new_video(args[:5], args[5], args[6], args[7:]),
        inputs=video_outputs + [audio_output, product_description] + lora_outputs,
